...
Requires [Docker Desktop](https://www.docker.com/products/docker-desktop/) (or Docker Engine + Compose). No manual container setup or MCP configuration.

**No tokens or `.env.local` required** to open the app UI. Analyze/summarize need Cursor webhooks later (see Setup above).

Clone, enter the repo, and start app + Postgres with one command:

```bash
git clone https://github.com/matan-moshe-art/-APP-.git
cd ./-APP-    # use ./ if the folder name starts with -
docker compose up --build
```

Open [http://localhost:3000](http://localhost:3000).

Optional — enable analyze/summarize API features (after UI loads):
...
If you cloned before Docker files were pushed, run `git pull` then `docker compose up --build` again.
