...
function isValidResult(v: unknown): v is SummarizeResult {
...
function extractResult(raw: unknown): SummarizeResult | null {
  const queue: unknown[] = [raw];
  const visited = new Set<unknown>();
...
export async function POST(request: Request) {
  ...
  const result: SummarizeResult | null = extractResult(body);
    ...
    return NextResponse.json(
      {
        ...
        expected:
          "{ correlationId, topic, urgency, actions, recommendations }",
      },
      { status: 400 },
    );
  }

  const found = completeSummarize(correlationId, result);
  if (!found) {
    return NextResponse.json(
